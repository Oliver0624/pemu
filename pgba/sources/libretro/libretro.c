...
static size_t _width = VIDEO_WIDTH_MAX;
struct VDir* _archive = NULL;
...
bool retro_load_game(const struct retro_game_info* game) {
	...
	if (game->data) {
		...
		if ( 0 == strcasecmp(&game->path[strlen(game->path) - 4], ".zip")
			|| 0 == strcasecmp(&game->path[strlen(game->path) - 3], ".7z") ) {
			// search in archive
			_archive = VDirOpenArchive(game->path);
			if (!_archive) {
				return false;
			}

			struct VDirEntry* dirent = _archive->listNext(_archive);
			while (dirent) {
				rom = _archive->openFile(_archive, dirent->name(dirent), O_RDONLY);
				if (!rom) {
					dirent = _archive->listNext(_archive);
					continue;
				}
				core = mCoreFindVF(rom);
				if (core) {
	...
	core->init(core);
	core->setVideoBuffer(core, outputBuffer, _width);
...
void retro_set_video_buffer(void *buff, size_t width) {
	if (outputBuffer != buff) {
		outputBuffer = buff;
		_width = width;
	}

	if (core && outputBuffer) {
		core->setVideoBuffer(core, outputBuffer, _width);
	}
}

void retro_set_savedate(void* savedata_out) {
	savedata = savedata_out;
}